...
class CreateMetadata(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["analysis_id", "binary_id", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Uploaded', 'Queued', 'Processing', 'Complete', 'Error', 'unknown_default_open_api']):
            raise ValueError("must be one of enum values ('Uploaded', 'Queued', 'Processing', 'Complete', 'Error', 'unknown_default_open_api')")
        return value

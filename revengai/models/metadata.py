...
class Metadata(BaseModel):
    ...
    Metadata
    """ # noqa: E501
    log_history: Optional[List[Annotated[List[Any], Field(min_length=2, max_length=2)]]] = Field(description="Progress messages the run recorded, oldest first, empty until the run logs anything. Each entry is a [timestamp, message] pair.")
    status: StrictStr = Field(description="Run status. UNINITIALISED means this agent has never been triggered for the analysis, so it is safe to start one.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["log_history", "status"]

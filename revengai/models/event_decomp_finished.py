...
class EventDecompFinished(BaseModel):
    """
    EventDecompFinished
    """ # noqa: E501
    ...
    def event_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['decomp_finished', 'unknown_default_open_api']):
            raise ValueError("must be one of enum values ('decomp_finished', 'unknown_default_open_api')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "data": DecompFinishedEvent.from_dict(obj["data"]) if obj.get("data") is not None else None,
            "event": obj.get("event"),
            "id": obj.get("id"),
            "retry": obj.get("retry")
        })
